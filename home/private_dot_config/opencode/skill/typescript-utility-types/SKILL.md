...
type CSSProperty = "color" | "background-color" | "font-size" | "margin-top";
...
// Curry function type
type Curry<T> = T extends (arg: infer A, ...args: infer R) => infer Return
  ? (arg: A) => R extends [] ? Return : Curry<(...args: R) => Return>
  : never;
...
type Builder<T, R = {}> = {
  [K in keyof T]: (value: T[K]) => Builder<Omit<T, K>, R & Pick<T, K>>;
} & (R extends T ? { build(): T } : {});
